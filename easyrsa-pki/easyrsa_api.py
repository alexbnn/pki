...
def create_temp_vars_for_cert():
    """Create temporary vars file without EASYRSA_REQ_CN for certificate operations"""
    try:
        # Check for backup vars file first, then original
        backup_vars = os.path.join(PKI_PATH, "vars.backup")
        original_vars = os.path.join(PKI_PATH, "vars")
        
        source_file = backup_vars if os.path.exists(backup_vars) else original_vars
        temp_vars_content = ""
        
        if os.path.exists(source_file):
            with open(source_file, 'r') as f:
                for line in f:
                    # Skip the EASYRSA_REQ_CN line to avoid conflicts
                    if not line.strip().startswith('set_var EASYRSA_REQ_CN'):
                        temp_vars_content += line
        
        # Create temporary vars file
        temp_vars_path = os.path.join(PKI_PATH, "vars.tmp")
        with open(temp_vars_path, 'w') as f:
            f.write(temp_vars_content)
        
        print(f"Created temporary vars file at: {temp_vars_path} from source: {source_file}")
        return temp_vars_path
        
    except Exception as e:
        print(f"Error creating temporary vars file: {e}")
        raise
...
def show_ca():
    ...
    try:
        ...
        if not os.path.exists(ca_file_path):
            return jsonify({
                "status": "error",
                "message": "Certificate Authority not found. Initialize PKI and build CA first."
            }), 404
        ...
        if result.returncode == 0:
            return jsonify({
                ...
                "stdout": result.stdout,
                "stderr": result.stderr,
                "message": "CA details retrieved successfully"
            })
        else:
            return jsonify({
                "status": "error",
                ...
                "stdout": result.stdout,
                "stderr": result.stderr,
                "message": "Failed to retrieve CA details"
            })
            
    except Exception as e:
        print(f"Error in show_ca: {e}")
        return jsonify({
            "status": "error",
            "message": f"Failed to show CA: {str(e)}"
        }), 500
...
def sign_req(params):
    ...
    # If certificate was signed successfully, delete the request file
    if result.returncode == 0:
        request_file = os.path.join(PKI_PATH, "reqs", f"{name}.req")
        if os.path.exists(request_file):
            try:
                os.remove(request_file)
                print(f"Cleaned up request file after signing: {request_file}")
            except Exception as e:
                print(f"Warning: Could not delete request file {request_file}: {e}")
...
def build_client_full(params):
    ...
    # Temporarily rename the original vars file to avoid CN conflicts
    original_vars = os.path.join(PKI_PATH, "vars")
    backup_vars = os.path.join(PKI_PATH, "vars.backup")
    temp_vars_path = None
    
    try:
        # Backup original vars file
        if os.path.exists(original_vars):
            shutil.move(original_vars, backup_vars)
        
        # Create a clean vars file without EASYRSA_REQ_CN
        temp_vars_path = create_temp_vars_for_cert()
        shutil.move(temp_vars_path, original_vars)
        ...
        # Remove any CN-related variables that might conflict
        clean_env.pop('EASYRSA_REQ_CN', None)
        
        print(f"Building client certificate for {name} with clean vars file")
        result = run_easyrsa_command(['build-client-full', name, 'nopass'], None, clean_env)
        
        # If certificate was created successfully, delete the request file
        if result.returncode == 0:
            request_file = os.path.join(PKI_PATH, "reqs", f"{name}.req")
            if os.path.exists(request_file):
                try:
                    os.remove(request_file)
                    print(f"Cleaned up request file: {request_file}")
                except Exception as e:
                    print(f"Warning: Could not delete request file {request_file}: {e}")
        
    finally:
        # Restore original vars file
        if os.path.exists(original_vars):
            os.remove(original_vars)
        if os.path.exists(backup_vars):
            shutil.move(backup_vars, original_vars)
        if temp_vars_path and os.path.exists(temp_vars_path):
...
def build_server_full(params):
    ...
    try:
        ...
        print(f"Building server certificate for {name} with clean vars file")
        result = run_easyrsa_command(['build-server-full', name, 'nopass'], None, clean_env)
